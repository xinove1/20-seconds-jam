...
#ifdef HOT_RELOAD
# ifdef _WIN32
#  define GAMELIBEXTENSION ".dll"
#  include <raylib_win32.h>
#  include <windows.h>
#  include <conio.h>
# else
#  define GAMELIBEXTENSION ".so"
#  include <dlfcn.h>
#  include <string.h>
#  include <errno.h>
#  include <sys/stat.h>
# endif
 ...
 global cstr *LibGamePath = "./build/game"GAMELIBEXTENSION;
...
internal void update_and_draw()
{
	...
	BeginDrawing(); {
		...
		DrawTexturePro(ScreenTexture.texture,
		 ...
		 0.0f,
		 WHITE);
		DrawText(TextFormat("%d", GetFPS()), 30, 30, 30, RED); // TODO  PUT UNDER CONFIG
	} EndDrawing();
}
...
{
	#ifdef _WIN32
		local HMODULE libgame = NULL;
		if (libgame != NULL) {
			FreeLibrary(libgame);
		}

		CopyFile(LibGamePath, "./build/_game.dll", 0);
		libgame = LoadLibraryA("./build/_game.dll");
		if (libgame == NULL) {
			TraceLog(LOG_FATAL, "COULDNT LOAD %s, ERROR: %lu", LibGamePath, GetLastError());
		}
		LibGameTime = get_stat_time(LibGamePath);

		TraceLog(LOG_INFO, "(Re)Loaded gamelib");
		game->init_pre_raylib = (GameConfig (*)(void **data))GetProcAddress(libgame, "init_pre_raylib");
		game->init_pos_raylib = (void (*)(void))GetProcAddress(libgame, "init_pos_raylib");
		game->update = (b32 (*)(void))GetProcAddress(libgame, "update");
		game->draw = (void (*)(void))GetProcAddress(libgame, "draw");
		game->pos_reload = (void (*)(void *data))GetProcAddress(libgame, "pos_reload");
		game->pre_reload = (void (*)(void))GetProcAddress(libgame, "pre_reload");
