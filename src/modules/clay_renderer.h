...
void ClayRendererClose();
void ClayRendererInit(Clay_Arena *mem, Font *fonts);
void ClayRendererRender(Clay_RenderCommandArray renderCommands, Font* fonts);
...
internal Ray GetScreenToWorldPointWithZDistance(Vector2 position, Camera camera, int screenWidth, int screenHeight, float zDistance)
{
    Ray ray = { 0 };
...
internal Clay_Dimensions Raylib_MeasureText(Clay_StringSlice text, Clay_TextElementConfig *config, void *userData) {
    ...
    Font fontToUse = GetFontDefault(); // TODO Change this?
    if (fonts) {
...
internal void HandleClayErrors(Clay_ErrorData errorData) {
...
void ClayRendererInit(Clay_Arena *mem, Font *fonts)
{
    // TODO  Deal with error of needing more memory
    if (mem->capacity == 0) {
        u64 totalMemorySize = Clay_MinMemorySize();
        *mem = Clay_CreateArenaWithCapacityAndMemory(totalMemorySize, malloc(totalMemorySize));
    }
    Clay_Initialize(*mem, (Clay_Dimensions) { (float)GetScreenWidth(), (float)GetScreenHeight() }, (Clay_ErrorHandler) { HandleClayErrors, 0 });
