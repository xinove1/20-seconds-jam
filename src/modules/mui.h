...
b32 MUiSaveStyle(mu_Context *ctx, cstr *file) 
{
	...
	// NOTE  Changing from void do size because msvc trows error of unknow size
	b32 r = SaveFileData(file, (&ctx->style->size), sizeof(mu_Style) - ((size *)&ctx->style->size - (size *)ctx->style));
	return (r);
}
...
b32 MUiLoadStyle(mu_Context *ctx, cstr *file)
{
	...
	i32 data_read = 0;
	// NOTE  Changing from void do size because msvc trows error of unknow size
	i32 data_expected = sizeof(mu_Style) - ((size *)&ctx->style->size - (size *)ctx->style);
